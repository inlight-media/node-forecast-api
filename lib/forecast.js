...
Forecast.prototype.addAssignment = function(assignment, callback) {
	this._request('/assignments', { method: 'POST', body: { assignment: assignment } }, function(err, json) {
		if (err) {
			return callback(err.errors[0]);
		}
		callback(null, json.assignment);
	});
};
